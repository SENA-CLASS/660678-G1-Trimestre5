...
import edu.co.sena.onlineshop.integracion.jpa.entities.ItemPK;
...
public class ItemJpaController implements Serializable {
    ...
    public void create(Item item) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            em.persist(item);
            if (pedido != null) {
                pedido.getItemCollection().add(item);
                pedido = em.merge(pedido);
            }
            if (producto != null) {
                producto.getItemCollection().add(item);
    ...
    public void edit(Item item) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (pedidoOld != null && !pedidoOld.equals(pedidoNew)) {
                pedidoOld.getItemCollection().remove(item);
                pedidoOld = em.merge(pedidoOld);
            }
            if (pedidoNew != null && !pedidoNew.equals(pedidoOld)) {
                pedidoNew.getItemCollection().add(item);
                pedidoNew = em.merge(pedidoNew);
            }
            if (productoOld != null && !productoOld.equals(productoNew)) {
                productoOld.getItemCollection().remove(item);
                productoOld = em.merge(productoOld);
            }
            if (productoNew != null && !productoNew.equals(productoOld)) {
                productoNew.getItemCollection().add(item);
    ...
    public void destroy(ItemPK id) throws NonexistentEntityException {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            ...
            Pedido pedido = item.getPedido();
            if (pedido != null) {
                pedido.getItemCollection().remove(item);
                pedido = em.merge(pedido);
            }
            Producto producto = item.getProducto();
            if (producto != null) {
                producto.getItemCollection().remove(item);
                producto = em.merge(producto);
            }
            em.remove(item);
            em.getTransaction().commit();
