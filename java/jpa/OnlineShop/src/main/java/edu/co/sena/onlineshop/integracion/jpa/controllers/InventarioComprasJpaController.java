...
import edu.co.sena.onlineshop.integracion.jpa.entities.InventarioComprasPK;
...
public class InventarioComprasJpaController implements Serializable {
    ...
    public InventarioComprasJpaController(EntityManagerFactory emf) {
        this.emf = emf;
    }
    ...
    public void create(InventarioCompras inventarioCompras) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            if (producto != null) {
                producto.getInventarioComprasCollection().add(inventarioCompras);
                producto = em.merge(producto);
            }
            if (proveedor != null) {
                proveedor.getInventarioComprasCollection().add(inventarioCompras);
    ...
    public void edit(InventarioCompras inventarioCompras) throws NonexistentEntityException, Exception {
        inventarioCompras.getInventarioComprasPK().setProductoIdProducto(inventarioCompras.getProducto().getIdProducto());
        inventarioCompras.getInventarioComprasPK().setProveedorNumDocumento(inventarioCompras.getProveedor().getProveedorPK().getNumDocumento());
        inventarioCompras.getInventarioComprasPK().setProveedorTipoDocumento(inventarioCompras.getProveedor().getProveedorPK().getTipoDocumento());
        EntityManager em = null;
        try {
            ...
            if (productoOld != null && !productoOld.equals(productoNew)) {
                productoOld.getInventarioComprasCollection().remove(inventarioCompras);
                productoOld = em.merge(productoOld);
            }
            if (productoNew != null && !productoNew.equals(productoOld)) {
                productoNew.getInventarioComprasCollection().add(inventarioCompras);
                productoNew = em.merge(productoNew);
            }
            if (proveedorOld != null && !proveedorOld.equals(proveedorNew)) {
                proveedorOld.getInventarioComprasCollection().remove(inventarioCompras);
                proveedorOld = em.merge(proveedorOld);
            }
            if (proveedorNew != null && !proveedorNew.equals(proveedorOld)) {
                proveedorNew.getInventarioComprasCollection().add(inventarioCompras);
    ...
    public void destroy(InventarioComprasPK id) throws NonexistentEntityException {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            ...
            Producto producto = inventarioCompras.getProducto();
            if (producto != null) {
                producto.getInventarioComprasCollection().remove(inventarioCompras);
                producto = em.merge(producto);
            }
            Proveedor proveedor = inventarioCompras.getProveedor();
            if (proveedor != null) {
                proveedor.getInventarioComprasCollection().remove(inventarioCompras);
                proveedor = em.merge(proveedor);
            }
